...
**Deviation (mech-pencil, recorded here per the rule above).** Its
distribution model is unchanged (still "bun as dep" + bin stub), but
its `build` script is `tsup && bun build ./src/cli.ts …` rather than
plain `tsup`. Reason: mech-pencil embeds its committed HeroUI `.pen`
library via `import … with { type: "file" }`
(`frameworks/heroui/library-embed.ts`, regenerated by `build-library`)
so the CLI works as a future `bun build --compile` binary. esbuild
(tsup) can't process that import attribute; `bun build` can. tsup
still builds the `.pen`-free library entry (`src/index.ts` + dts).
This is the embed prerequisite from the Bun-distribution analysis; the
`--compile` flip + cross-platform strategy remain a separate decision.
