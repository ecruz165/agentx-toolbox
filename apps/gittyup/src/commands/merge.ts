    ...
    .action(
      async (
        ...
        const targets: MergeTarget[] = repos.map((repo) => ({
          repo,
          sourceBranch: repo.branches[source] ?? source,
          targetBranch: repo.branches[target] ?? target,
        }));

        console.log(chalk.bold('\n  Merge Plan:'));
        for (const t of targets)
          console.log(chalk.dim(`    ${t.repo.name}: ${t.sourceBranch} → ${t.targetBranch}`));

        const ok = await confirm({
          message: `Proceed with merge across ${targets.length} repo(s)?`,
          default: true,
        });
        if (!ok) return;

        await orchestrator.executeMerge(targets, {
