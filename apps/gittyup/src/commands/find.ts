...
import { type DiscoveredRepo, RepoFinder } from '../core/repo-finder.js';
import {
  BACK,
  checkboxWithBack,
  groupAssigner,
...
export function registerFind(program: Command): void {
  program
    ...
    .action(async (directory: string, opts: { depth: string; metadata?: boolean }) => {
      ...
      // biome-ignore lint/suspicious/useIterableCallbackReturn: finder.find is a custom search method, not Array.prototype.find — callback is a progress reporter
        ...
        switch (state) {
          ...
          case 'select': {
            ...
            if (selectionMode === 'folder') {
              const folderChoices = Array.from(folderGroups.entries()).map(
                ([folder, folderRepos]) => ({
              ...
              const choices: Array<
                { name: string; value: RepoChoice; checked: boolean; group?: string } | Separator
              > = [];
              for (const [folder, folderRepos] of folderGroups) {
