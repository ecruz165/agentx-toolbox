  ...
  program
    ...
    .action(
      async (opts: {
        ...
        if (!scope) {
          console.error(chalk.red('Specify --group or --repo'));
          process.exit(1);
        }
        if (!opts.source || !opts.target) {
          console.error(chalk.red('Specify --source and --target branches'));
          process.exit(1);
        }
        ...
        for (const repo of repos) {
          ...
          if (opts.commits) {
            ...
            if (commits.length === 0) {
              console.log(chalk.yellow(`  Skipping ${repo.name}`));
              continue;
            }
          } else {
            console.error(chalk.red('Specify --commits or use --interactive'));
            process.exit(1);
          }
        ...
        for (const t of targets) {
          console.log(
            chalk.dim(
              `    ${t.repo.name}: ${t.commits.length} commit(s) ${t.sourceBranch} → ${t.targetBranch}`,
            ),
          );
        ...
        const ok = await confirm({
          message: `Proceed with cherry-pick across ${targets.length} repo(s)?`,
          default: true,
        });
        if (!ok) return;

        await orchestrator.executeCherryPick(targets, {
          aiMode: (opts.ai as AiMode) ?? undefined,
          push: opts.push,
          createPR: opts.pr,
          fetch: opts.fetch,
        });
