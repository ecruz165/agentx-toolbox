...
import { submitContribution } from "../lib/api/contribute.js";
import type { ContributionReviewer } from "../lib/api/validation.js";
import { loadCoreTags } from "../lib/tags.js";
...
export function createApp(options: AppOptions): Hono {
  ...
  app.get("/api/v1/search", async (c) => {
    ...
    const skills = index.skills.filter(
      (s) =>
        matches(s.name) || matches(s.description) || matchesTag(s.tags),
    );
  ...
  if (writable) {
    ...
    app.post("/api/v1/contributions", async (c) => {
      ...
      c.header(
        "Location",
        `/api/v1/contributions/${encodeURIComponent(result.response.id)}`,
      );
    ...
    app.get("/api/v1/contributions/:id", async (c) => {
      ...
      const versions = await listVersionsByKind(
        writableStorage,
        parsed.kind,
        parsed.slug,
      );
  ...
  // ── 404 fallback ──────────────────────────────────────────────
  app.notFound((c) => apiError(c, 404, "not_found", `No handler for ${c.req.method} ${c.req.path}`));
...
async function listVersionsByKind(
