...
describe("runInit — overwrite protection", () => {
  it("refuses to overwrite existing config without --force", () => {
    ...
    expect(() => runInit({ mode: "standalone", email: "a@b.c" })).toThrow(
...
describe("validateInitOptions", () => {
  it("rejects invalid email", () => {
    expect(() =>
      validateInitOptions({ mode: "standalone", email: "not-an-email" }),
    ).toThrow(/Invalid email/);
  });

  it("rejects invalid mode", () => {
    expect(() =>
      validateInitOptions({ mode: "neither" as never, email: "a@b.c" }),
  ...
  it("accepts standalone with just mode + email", () => {
    expect(() =>
      validateInitOptions({ mode: "standalone", email: "a@b.c" }),
    ).not.toThrow();
  });
