...
```
1. /core:frameworks:heroui:components:surfaces       (Card, Container)
2. /core:frameworks:heroui:components:buttons        (depends on surfaces, motion, a11y)
3. /core:frameworks:heroui:components:forms          (depends on buttons, a11y)
4. /core:frameworks:heroui:components:selection      (depends on forms)
5. /core:frameworks:heroui:components:feedback       (depends on motion, a11y)
6. /core:frameworks:heroui:components:overlays       (depends on motion, z-index, a11y)
7. /core:frameworks:heroui:components:navigation     (depends on buttons, surfaces)
8. /core:frameworks:heroui:components:data-display   (depends on surfaces, density)
9. /core:frameworks:heroui:components:date-time      (depends on selection, i18n)
10. /core:frameworks:heroui:components:color-system  (visual reference)
11. /core:frameworks:heroui:components:charts        (depends on surfaces, colors)
12. /core:frameworks:heroui:components:media         (depends on surfaces, motion)
```

This phase produces both `.pen` files AND React components (via
`/core:frameworks:heroui:build-components --foundation-only`). The React must exist
...
After all patterns generate, run `/core:frameworks:heroui:build-components
...
`/core:frameworks:heroui:build-components <page-slug>` after scaffold completes.
