---
outcome: Slice into release backbones
description: Define release slices on an existing story map. Add new slices (v1.1, v1.2, etc.), modify existing slices, or update slice status. Each slice is a thin-but-complete user journey for a release. Optional integration with /core:integrations:jira for bulk ticket creation from slice.
...
if [ -z "$INTEGRATION" ]; then
  ...
  echo "  /core:integrations:setup jira"
  echo "  /core:integrations:setup github"
  echo "  /core:integrations:setup linear"
  exit 0
fi
```
...
for STORY_ID in $(echo "$MVP_STORIES_WITHOUT_TICKET"); do
  ...
  # Invoke integration
  TICKET_REF=$(/core:integrations:jira create-issue \
    --type Story \
    --title "$TITLE" \
