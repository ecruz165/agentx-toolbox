...
if [ "$ACTIVE" != "true" ]; then
  ...
  echo "run /core:frameworks:init to detect and activate."
  exit 0  # not a failure; just not applicable
fi

# Storybook config exists?
test -f product/.pencil-storybook.json || {
  echo "Storybook deep config missing. Run /core:frameworks:storybook:init"
  exit 1
}
...
[ "$PLAYWRIGHT" = "true" ] || {
  echo "Playwright required for drift detection. Run /core:tools:setup playwright"
  exit 1
}
```
...
/core:frameworks:storybook:verify:health "$SCOPE" --json > /tmp/sd-1.json
...
/core:frameworks:storybook:catalog --format json > /tmp/sd-2.json
...
if [ -z "$A11Y_ADDON" ]; then
  ...
  /core:frameworks:storybook:verify:a11y "$SCOPE" --severity "$SEVERITY" --json > /tmp/sd-4.json
...
if [ "$CHROMATIC_ENABLED" != "true" ] || [ "$INCLUDE_CHROMATIC" != "true" ]; then
  ...
  /core:frameworks:storybook:chromatic --json > /tmp/sd-5.json
...
if [ -z "$SCOPE" ] || [ "$SCOPE" = "all" ]; then
  /core:frameworks:storybook:verify:deploy --no-rebuild --json > /tmp/sd-7.json 2>/dev/null || true
...
Routing:
  Auto-fixable (with --fix):
    SD-1 broken stories:       3 → /core:frameworks:storybook:verify:fix
    SD-2 coverage gaps:        2 → /core:frameworks:storybook:stories:gen-missing
    SD-6 orphaned story files: 1 → confirm and remove
    SD-3 stale MDX:            7 → /core:frameworks:storybook:stories:doc --update
...
for STORY_ID in $BROKEN_STORIES; do
  ...
  /core:frameworks:storybook:verify:fix "$STORY_ID"
...
/core:frameworks:storybook:stories:gen-missing "$SCOPE"
```
...
for STALE_ENTRY in "${SD3_STALE[@]}"; do
  ...
  /core:frameworks:storybook:stories:doc "$COMPONENT" --update --sections api
done
```
...
/core:frameworks:storybook:verify:health "$SCOPE" --json > /tmp/post-remediation-health.json
