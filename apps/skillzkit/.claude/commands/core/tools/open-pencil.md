...
exporting specs) lives in `/core:tools:pencil-mcp`. This tool is
...
1. Verify open-pencil active in tools manifest:
   ...
   if [ "$ACTIVE" != "true" ]; then
     echo "open-pencil not active. Run /core:tools:setup open-pencil"
     exit 1
   fi
   ```
...
The Pencil app window should appear shortly. If it doesn't:
  ...
  - Try /core:tools:open-pencil --new to open without a file
```
...
Suggestions:
  ...
  - Run /core:tools:open-pencil --new to open Pencil without a file
```
...
## Distinction from /core:tools:pencil-mcp

| Tool | Purpose |
|------|---------|
| `/core:tools:open-pencil` | Open the Pencil app (UI; user interacts) |
| `/core:tools:pencil-mcp` | Query structured design data (components, styles, frames) without opening UI |
...
/core:tools:open-pencil designs/skoolscout-design-system.pencil

# Open Pencil with no file (recent files / new)
/core:tools:open-pencil --new

# Resolved from manifest
/core:tools:open-pencil "open my design system"

# From template (if Pencil version supports)
/core:tools:open-pencil --from-template skoolscout-page-template
```
...
`/core:tools:pencil-mcp`) covers structured data operations.
