...
1. Verify context7 active in tools manifest:
   ...
   if [ "$ACTIVE" != "true" ]; then
     echo "Context7 not active. Run /core:tools:setup context7"
     exit 1
   fi
   ```
...
for a library, they invoke `/core:tools:context7` to get fresh
...
| GitHub search via /core:integrations:github | Code examples in real repos |
...
```bash
# Library docs lookup
/core:tools:context7 "how do I use server components in Next.js 15"

# Specific topic
/core:tools:context7 "Tailwind v4 @theme directive"

# Migration guidance
/core:tools:context7 "Spring Boot 2 to 3 migration breaking changes"

# Specific version
/core:tools:context7 "React 19 useOptimistic hook" --version "19"

# Compact lookup
/core:tools:context7 "Embabel STRIPS planner config" --tokens 2000
```
