...
to `/core:tools:open-pencil` (which opens the UI).
...
1. Verify pencil-mcp active in tools manifest:
   ...
   if [ "$ACTIVE" != "true" ]; then
     echo "Pencil MCP not active. Run /core:tools:setup pencil-mcp"
     exit 1
   fi
   ```
...
Pencil MCP is the structured-data side; `/core:tools:open-pencil`
is the UI side. Workflows often compose both.

## What this tool does NOT do

- **Open the Pencil app.** Use `/core:tools:open-pencil`.
...
- **Replace `/core:integrations:figma`.** If your team uses Figma,
  use `/core:integrations:figma`. Pencil and Figma are alternatives;
...
/core:tools:pencil-mcp "list components"

# Specific component spec
/core:tools:pencil-mcp "show me the Button component spec"

# Design tokens
/core:tools:pencil-mcp "list color tokens defined in our design system"

# Cross-references
/core:tools:pencil-mcp "where is the SearchBar component used"

# Specific file (overriding default)
/core:tools:pencil-mcp --file marketing-site.pencil "list pages"
```
...
For UI launching, see `/core:tools:open-pencil`.
...
- Pencil app or Pencil file access (depends on MCP server
  implementation)
- /core:tools:open-pencil (sister tool for UI launching)
