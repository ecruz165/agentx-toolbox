...
1. Verify imagemagick active in tools manifest:
   ...
   if [ "$ACTIVE" != "true" ]; then
     echo "ImageMagick not active. Run /core:tools:setup imagemagick"
     exit 1
   fi
   ```
...
```bash
# Format conversion
/core:tools:imagemagick "convert /tmp/input.heic to PNG saved at /tmp/output.png"

# Resize
/core:tools:imagemagick "resize /tmp/large.png to 800px wide preserving aspect ratio, save as /tmp/small.png"

# Color palette extraction
/core:tools:imagemagick "show me the top 10 colors in /tmp/design-system.png"

# Visual diff with metrics
/core:tools:imagemagick "compare /tmp/before.png and /tmp/after.png; report MSE and SSIM"

# Histogram
/core:tools:imagemagick "generate color histogram for /tmp/dashboard.png"
```
...
- /core:frameworks:storybook:migration:verify (Loop 5; optional)
- /core:frameworks:storybook:verify:health (blank detection;
  optional)
