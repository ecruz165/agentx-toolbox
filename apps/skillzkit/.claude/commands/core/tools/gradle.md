...
1. Verify gradle active in tools manifest:
   ...
   if [ "$ACTIVE" != "true" ]; then
     echo "Gradle not active. Run /core:tools:setup gradle"
     exit 1
   fi
   ```
...
To investigate a specific conflict:
  /core:tools:gradle "explain why jackson-databind is included"
...
```bash
# Standard build
/core:tools:gradle "build the project"

# Skip tests (upgrade verification)
/core:tools:gradle "build skipping tests" -x test

# Show outdated dependencies
/core:tools:gradle "show me available dependency updates"

# Module build
/core:tools:gradle "build only the api module"

# Dependency insight
/core:tools:gradle "explain why jackson-databind is included"

# Run OpenRewrite recipes
/core:tools:gradle "run OpenRewrite recipes"
```
