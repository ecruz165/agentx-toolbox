...
1. Read `product/.pencil-tools.json`. Verify `playwright` is
   ...
   if [ "$ACTIVE" != "true" ]; then
     echo "Playwright not active. Run /core:tools:setup playwright"
     exit 1
   fi
   ```
...
direct user invocation of `/core:tools:playwright` is for ad-hoc
...
```bash
# Screenshot a URL
/core:tools:playwright "screenshot localhost:6006/iframe.html?id=button--default to /tmp/button.png"

# Element screenshot
/core:tools:playwright "screenshot the .hero-banner element on https://skoolscout.com to /tmp/hero.png at 1440x900"

# Capture page errors
/core:tools:playwright "load localhost:6006/iframe.html?id=card--broken and capture any console errors"

# Run an interaction
/core:tools:playwright "load localhost:6006/iframe.html?id=button--clickable, click the button, verify the count incremented"

# Performance metrics
/core:tools:playwright "load https://skoolscout.com and capture Core Web Vitals"
```
