...
splunk (instance: security):
  SPLUNK_TOKEN (keychain):    ✗ NOT FOUND
                              ...
                              Action: /core:integrations:setup splunk --instance security
...
Action items:
  1. Rotate jira/JIRA_API_TOKEN soon (92 days, threshold 90)
     ...
     Then: /core:integrations:setup jira
...
jira/JIRA_API_TOKEN:
  ...
  Action: Generate new API token, then run /core:integrations:setup jira
...
splunk/SPLUNK_TOKEN (security):
  ...
  Action: Generate new token, then run /core:integrations:setup splunk --instance security
```
...
```bash
/core:integrations:credentials --revoke jira JIRA_API_TOKEN
```
...
Revocation will:
  ...
  - Subsequent invocations of /core:integrations:jira will fail
    until re-setup
...
DOCTOR_OUTPUT=$(/core:integrations:credentials --doctor --json --integration jira 2>/dev/null)
...
if [ "$HAS_ALL_JIRA_CREDS" != "true" ]; then
  ...
  echo "Run /core:integrations:credentials --doctor for diagnostics"
  exit 1
fi
```
...
/core:integrations:credentials --rotation-due --json | \
...
/core:integrations:credentials --rotation-due

# Specific integration
/core:integrations:credentials --doctor --integration jira
