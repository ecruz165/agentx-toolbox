...
```bash
/core:integrations:setup jira --check
```
...
```bash
/core:integrations:setup jira --update
```
...
```bash
/core:integrations:setup jira --reset
```
...
/core:integrations:setup github --instance enterprise
```
...
Next steps:
  - Use directly: /core:integrations:jira "<prompt>"
  - Verify health: /core:integrations:credentials --doctor
  - Re-verify periodically: /core:integrations:setup jira --check
...
- **Auto-rotate credentials.** Surfaces rotation reminders via
  `/core:integrations:credentials --doctor` but doesn't rotate.
