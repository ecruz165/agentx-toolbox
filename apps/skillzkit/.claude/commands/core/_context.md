...
- **`core/tools/`** — local-invocation tools (build, lint,
  browser, image, package managers, infra)
- **`core/integrations/`** — remote service invocations
  (issue trackers, code hosting, design, messaging,
  observability)
- **`core/workflows/`** — workflow management state machine
  (`/core:workflows:manage`) and the unified workflow index
  (`/core:workflows:_index`); per-domain workflow playbooks
  themselves still live under each persona's `workflows/`
  folder
- **`core/frameworks/`** — framework bindings (HeroUI,
  Storybook; future: MUI, Chakra, etc.) that bridge design
  artifacts to concrete component implementations
- Future: `core/schemas/` (manifest schemas, currently at suite
  root)

The principle: anything that crosses every persona belongs in
core. Audit, tools, integrations, workflow management, and
framework bindings all qualify — every persona reads from them
or runs through them.
...
- **Workflow playbooks** — workflows live in their primary
  ...
  (in `core/workflows/`); the playbooks aren't.
