...
storybook: ⚠ DRIFT
        ...
        Run /core:frameworks:init --update to refresh.
```
...
✓ storybook activated. For full configuration including
  ...
  run /core:frameworks:storybook:init
...
Next steps:
  - Configure storybook deep settings: /core:frameworks:storybook:init
  - Build components: /core:frameworks:heroui:build-components
  - Or simply invoke any /core:frameworks:* command — bindings are ready.
```
...
Framework manifest not found. Running /core:frameworks:init to
detect and activate bindings...

[init runs as above]

Continuing with /core:frameworks:storybook:stories:gen Button...
```
...
storybook: ✗ NO LONGER DETECTED
        ...
        Otherwise, /core:frameworks:init --update to re-detect.
```
...
/core:frameworks:init --check

# Re-detect after adding new bindings
/core:frameworks:init --update

# Targeted update for one binding
/core:frameworks:init --update --binding storybook

# Component bindings only
/core:frameworks:init --check --scope component
```
