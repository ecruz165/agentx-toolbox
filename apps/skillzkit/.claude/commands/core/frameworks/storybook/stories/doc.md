...
2. Verify `@storybook/addon-docs` (or equivalent) is in the
   ...
   test -n "$DOCS_ADDON" || {
     ...
     echo "@storybook/addon-docs. Install and re-run /core:frameworks:storybook:init"
...
Same logic as `/core:frameworks:storybook:stories:gen` Phase 1. The component
...
test -f "$STORY_FILE" || {
  ...
  echo "Generate stories first via /core:frameworks:storybook:stories:gen"
  exit 1
}
```
...
- **Generate accessibility audit results.** That's
  `/core:frameworks:storybook:verify:a11y`. Doc generation captures known a11y
...
/core:frameworks:storybook:stories:doc Button

# Update existing MDX (refresh autodetected sections)
/core:frameworks:storybook:stories:doc Button --update

# Limit to specific sections
/core:frameworks:storybook:stories:doc Button --sections overview,usage

# Just the API reference (useful when component API changed)
/core:frameworks:storybook:stories:doc Button --update --sections api
```
