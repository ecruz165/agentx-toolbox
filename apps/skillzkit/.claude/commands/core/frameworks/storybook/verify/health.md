...
2. Verify `.pencil-storybook.json` exists with required fields
   (auto-trigger `/core:frameworks:storybook:init` for missing
   fields)
...
Next steps:
  - Run /core:frameworks:storybook:verify:fix <story-id> for each
    FAIL to investigate and apply fixes
  - Run /core:frameworks:storybook:verify:fix WARN to address
    warning-level issues
  - Re-run /core:frameworks:storybook:verify:health after fixes
```
...
- **Modify code.** It surfaces broken stories; fixing happens
  via `/core:frameworks:storybook:verify:fix` (or manual edits).
...
# Just atoms
/core:frameworks:storybook:verify:health atoms

# Specific component
/core:frameworks:storybook:verify:health Button

# Specific story
/core:frameworks:storybook:verify:health core-atoms-button--with-icon

# Quick check without screenshots (faster)
/core:frameworks:storybook:verify:health --no-screenshots

# Limited batch (incremental work)
/core:frameworks:storybook:verify:health --limit 20

# CI integration
/core:frameworks:storybook:verify:health --json > health.json
```
