...
Next steps:
  ...
  - Run health check: /core:frameworks:storybook:verify:health
  - Generate stories for components without them:
    /core:frameworks:storybook:stories:gen-missing
  - Build catalog report: /core:frameworks:storybook:catalog
```

## Auto-trigger from other commands

When another `/core:frameworks:storybook:*` command runs and the manifest is
...
Example: `/core:frameworks:storybook:verify:health` runs and the manifest has
...
- **Validate that Storybook actually works.** Init captures
  ...
  `/core:frameworks:storybook:verify:*`.
...
/core:frameworks:storybook:init --update

# Update only addon detection (after installing new addons)
/core:frameworks:storybook:init --update --scope addons

# Update only component organization (after restructuring)
/core:frameworks:storybook:init --update --scope organization
```
