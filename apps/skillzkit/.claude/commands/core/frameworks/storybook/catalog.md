...
CHROMATIC_DATA=$(/core:frameworks:storybook:chromatic --json 2>/dev/null)
...
NEXT STEPS:
  ...
  - Run /core:frameworks:storybook:stories:gen-missing for the
    6 components without stories
  - Run /core:frameworks:storybook:stories:doc <component> for
    high-priority undocumented components
  - Run /core:frameworks:storybook:verify:health to check overall
    quality across stories
  - Run /core:frameworks:storybook:chromatic for visual regression
...
```bash
/core:frameworks:storybook:catalog --scope atoms
# Only reports on atoms

/core:frameworks:storybook:catalog --scope Discover
# Only reports on Discover section

/core:frameworks:storybook:catalog --scope Button
...
- **Run quality checks.** Surfaces metrics; doesn't audit
  ...
  `/core:frameworks:storybook:verify:*` commands.
- **Generate missing stories.** Surfaces gaps; user runs
  `/core:frameworks:storybook:stories:gen-missing` to address.
...
/core:frameworks:storybook:catalog --format markdown > storybook-catalog.md

# Including Chromatic stats
/core:frameworks:storybook:catalog --include-chromatic

# JSON for CI / dashboards
/core:frameworks:storybook:catalog --format json > catalog.json
```
