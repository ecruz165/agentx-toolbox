...
if [ "$ACTIVE" != "true" ]; then
  ...
  echo "/core:frameworks:init to detect and activate it."
  exit 1
fi
```
...
framework-binding commands trigger `/core:frameworks:init` so the
...
The `/core:frameworks:init` command activates bindings (writes the
...
- **`lastDetected`** — when init last scanned for this binding.
  Updates whenever `/core:frameworks:init` runs (full or
  `--update` mode).
- **`lastVerified`** — when values were last checked against
  ...
  `/core:frameworks:init --check` runs.
- **`lastReviewed`** — when the user last explicitly confirmed
  this binding's activation. Updates only on full
  `/core:frameworks:init` runs (with explicit user confirmation)
  or `/core:frameworks:manifest --review`.

Three different thresholds drive three different actions:

| Threshold | Default | Action when exceeded |
|-----------|---------|---------------------|
| Verification staleness | 14 days | Soft warning in pre-flight: "values last verified $N days ago; run `/core:frameworks:init --check` to re-verify" |
...
`/core:frameworks:init` detects bindings from project state:
...
`/core:frameworks:init` activates bindings (the binary "is this
binding in use?" question). Per-binding inits configure them
(the "how do we operate it?" details). Users typically run
`/core:frameworks:init` once on a fresh project, then run each
...
`/core:frameworks:manifest --deactivate <binding>`.
...
```
/core:frameworks:init                            # detect and activate bindings
/core:frameworks:init --check                    # verify only
/core:frameworks:init --update                   # re-detect
/core:frameworks:manifest                        # show current state
/core:frameworks:manifest --query <jq-path>      # query specific field
/core:frameworks:manifest --review               # explicit revisitation
/core:frameworks:manifest --deactivate <binding> # remove a binding

/core:frameworks:heroui:build-components         # heroui binding command
/core:frameworks:storybook:init                  # storybook deep configuration
/core:frameworks:storybook:stories:gen Button    # storybook story generation
/core:frameworks:storybook:verify:health         # storybook verification
```
