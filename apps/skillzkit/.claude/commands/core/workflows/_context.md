...
**Mark complete**: `/core:workflows:manage complete <step-slug>`
...
`/core:workflows:manage`. The state file holds:
...
`/core:frameworks:heroui:build-components`, `/market:email:promotional`, etc.).
...
The `list` subcommand of `/core:workflows:manage` discovers workflows
