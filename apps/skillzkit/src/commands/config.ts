...
export function runConfigCommand(
  ...
  try {
    ...
    if (!value) {
      ...
      if (single === undefined) {
        console.error(`Field "${field}" is not set on this config (mode=${config.mode})`);
        process.exit(1);
      }
      console.log(single);
      return;
...
function printConfig(config: SkillzkitConfig, showSecrets: boolean): void {
  ...
  if (config.mode === 'team') {
    ...
    if (showSecrets) {
      ...
      console.log(`team.keyEncrypted.ciphertext = <${blob.ciphertext.length} chars base64>`);
    }
  }
}
...
function readField(config: SkillzkitConfig, field: string): string | undefined {
  switch (field) {
    ...
    default:
      throw new Error(`Unknown field "${field}". Try one of: mode, email, apiUrl, keyMasked`);
  }
}

function setField(config: SkillzkitConfig, field: string, value: string): SkillzkitConfig {
  switch (field) {
    case 'email': {
      if (config.mode === 'team') {
        throw new Error(
