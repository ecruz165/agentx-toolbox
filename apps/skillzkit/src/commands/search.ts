...
export function runSearch(query: string, options: SearchOptions = {}): void {
  ...
  const matchesAnyTag = (tags: string[] | undefined) => (tags ?? []).some(matches);

  const cmds = getCommands().filter(
    (c) =>
      c.kind === 'command' && (matches(c.slug) || matches(c.description) || matchesAnyTag(c.tags)),
  );
  const wfs = getWorkflows().filter(
    (w) => matches(w.qualifiedName) || matches(w.description) || matchesAnyTag(w.tags),
  );
  const skls = getSkills().filter(
    (s) => matches(s.name) || matches(s.description) || matchesAnyTag(s.tags),
  );
