...
import { expandGroupIds, type ResolvedItem, resolveInstallPlan } from '../resolve.js';
...
export function runInstall(slugs: string[], options: InstallOptions = {}): void {
  ...
  if (missing.length > 0) {
    ...
    console.error(`Run 'skillzkit search <query>' or 'skillzkit list' to find available items.`);
    process.exit(1);
  }
  ...
  if (options.dryRun) {
    const expansionNote =
      expanded.length !== slugs.length
        ...
        : '';
    console.log(`Install plan: ${summarizePlan(plan)} (${plan.length} files)${expansionNote}`);
    console.log(
  ...
  console.log(`✓ Installed ${result.installedFiles} file(s) to ${targetClaude}`);
  if (result.skippedExisting > 0) {
    console.log(`  (${result.skippedExisting} skipped — already exist; pass --force to overwrite)`);
  }
  console.log(
...
function summarizePlan(plan: readonly ResolvedItem[]): string {
  ...
  const parts: string[] = [];
  if (counts.workflow) parts.push(`${counts.workflow} workflow${counts.workflow === 1 ? '' : 's'}`);
