...
import type { AuthorIdentity, CreateContributionRequest } from '../api/contracts.js';
...
describe('GET /api/v1/contributions/:id', () => {
  it('returns the contribution by content-addressable id', async () => {
    ...
    const get = await app.fetch(
      new Request(url(`/api/v1/contributions/${encodeURIComponent(created.id)}`)),
    );
  ...
  it('returns 404 for unknown id', async () => {
    const { app } = makeApp();
    const res = await app.fetch(
      new Request(url(`/api/v1/contributions/${encodeURIComponent('command:nope:nope@9.9.9')}`)),
    );
  ...
  it('returns 400 for malformed id', async () => {
    const { app } = makeApp();
    const res = await app.fetch(new Request(url('/api/v1/contributions/garbage-no-at-sign')));
    expect(res.status).toBe(400);
  });
});
...
describe('POST /api/v1/contributions/:id/promote', () => {
  it('promotes a version, makes it visible in /catalog', async () => {
    ...
    const get = await app.fetch(
      new Request(
        url(`/api/v1/contributions/${encodeURIComponent('command:core:tools:my-tool@1.0.0')}`),
      ),
    );
  ...
  it('returns 404 when promoting an unknown version', async () => {
    ...
    const res = await app.fetch(
      new Request(
        url(`/api/v1/contributions/${encodeURIComponent('command:nope:nope@1.0.0')}/promote`),
        {
          method: 'POST',
          headers: { Authorization: 'Bearer dev-token' },
        },
