...
import { type ContributionReviewer, validateContribution } from './validation.js';
...
function integerIncrement(existing: string[]): string {
  const ints = existing.map((v) => Number.parseInt(v, 10)).filter((n) => !Number.isNaN(n));
  ...
  if (req.kind === 'command') {
    ...
    return {
      command: {
        ...
        references: [],
        referencedBy: [],
        requires: stringArrayFm(req.frontmatter, 'requires'),
        produces: stringArrayFm(req.frontmatter, 'produces'),
        dependsOn: [],
        dependents: [],
...
function stringFm(fm: Record<string, unknown>, key: string): string | undefined {
  const v = fm[key];
  return typeof v === 'string' ? v : undefined;
}

function stringArrayFm(fm: Record<string, unknown>, key: string): string[] | undefined {
  const v = fm[key];
...
async function listExistingVersions(req: CreateContributionRequest, storage: CatalogStorage) {
  if (req.kind === 'command') {
