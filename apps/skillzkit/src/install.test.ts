...
describe('installSlugs', () => {
  ...
  it("copies the requested slug's file", () => {
    ...
    expect(existsSync(join(target, '.claude/commands/core/tools/pixelmatch.md'))).toBe(true);
  });

  it('always installs infra files (audit, workflows, _context)', () => {
    installSlugs(new Set(['core:tools:pixelmatch']), loadCatalog(), packageRoot, target);
    expect(existsSync(join(target, '.claude/commands/_context.md'))).toBe(true);
    expect(existsSync(join(target, '.claude/commands/core/audit/audit.md'))).toBe(true);
    expect(existsSync(join(target, '.claude/commands/core/workflows/manage.md'))).toBe(true);
  });
  ...
  it('always installs all skills regardless of the requested set', () => {
    ...
    for (const skill of catalog.skills) {
      expect(existsSync(join(target, '.claude/skills', skill.path))).toBe(true);
    }
  });
  ...
  it('writes runtime manifests for picked tools and integrations', () => {
    ...
    const integrationsManifest = JSON.parse(
      readFileSync(join(target, 'product/.pencil-integrations.json'), 'utf8'),
    );
