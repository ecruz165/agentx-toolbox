...
import { findTaskById, isClosedState, validateTransition } from '../config/state-engine.js';
...
export function executeSetStatus(
  ...
  if (task.tags.includes('qa-review-needed') && isClosedState(states, newStatus) && !opts.force) {
    throw new Error(
