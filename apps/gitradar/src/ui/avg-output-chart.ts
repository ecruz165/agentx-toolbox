 ...
 */
export function renderAvgOutputChart(options: AvgOutputChartOptions): string {
  const {
    bars,
    segmentDefs,
    labelWidth = 14,
  ...
  const globalMax = Math.max(1, ...bars.map((b) => b.total), ...bars.map((b) => b.runningAvg));
  ...
  for (const bar of bars) {
    ...
    const labelStr = padRight(bar.label, labelWidth) + chalk.dim(` (${bar.headcount})`);

    // Render the stacked bar
    const barStr = renderBar(bar.segments, bar.total, globalMax, availableBarWidth, segmentDefs);

    // Calculate marker position
    const markerPos = Math.round((bar.runningAvg / globalMax) * availableBarWidth);
    ...
    if (markerPos <= barVisualWidth) {
      ...
      const remaining = availableBarWidth - barVisualWidth - gapToMarker;
      if (remaining > 0) {
    ...
    if (markerPos <= barVisualWidth && markerPos > 0) {
      ...
      barArea = barStr + ' '.repeat(gapAfterBar) + chalk.white.bold(markerChar);
      const afterMarker = availableBarWidth - barVisualWidth - gapAfterBar - 1;
      if (afterMarker > 0) {
