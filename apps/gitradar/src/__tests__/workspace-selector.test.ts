...
function makeSource(type: 'global' | 'project', path: string): RegistrySource {
  return {
    type,
    path,
...
describe('selectWorkspace', () => {
  ...
  const projectSource = makeSource('project', '/projects/foo/.agentx/repos.yml');
  ...
  it('returns null when preselected name not found', async () => {
    ...
    expect(result).toBeNull();
    expect(errorSpy).toHaveBeenCalledWith('Workspace "nonexistent" not found.');
    expect(mockSelect).not.toHaveBeenCalled();
  ...
  it('falls through to prompt when preselected name matches multiple workspaces', async () => {
    ...
    const result = await selectWorkspace([wsGlobal, wsProject], 'frontend');
