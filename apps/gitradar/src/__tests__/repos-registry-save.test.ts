...
function makeWorkspace(
  repos: Array<{ name: string; path: string; group: string }>,
): LoadedWorkspace {
...
describe('addReposToWorkspace', () => {
  ...
  it('skips repos with duplicate names', () => {
    const ws = makeWorkspace([{ name: 'frontend', path: '/repos/frontend', group: 'web' }]);
  ...
  it('keeps source registry in sync', () => {
    const ws = makeWorkspace([]);
    addReposToWorkspace(ws, [{ name: 'new-repo', path: '/repos/new-repo', group: 'default' }]);
  ...
  it('handles empty input', () => {
    const ws = makeWorkspace([{ name: 'existing', path: '/repos/existing', group: 'default' }]);
