...
describe('renderTable', () => {
  describe('column width distribution', () => {
    ...
    it('applies minWidth floors', () => {
      ...
      const _parts = headerPlain.split('');
  ...
  describe('ANSI-aware truncation', () => {
    it('truncates content that exceeds column width with ellipsis', () => {
      const columns: Column[] = [{ key: 'name', label: 'Name', width: 6 }];
    ...
    it('truncates ANSI-colored content correctly', () => {
      const columns: Column[] = [{ key: 'bar', label: 'Bar', width: 5 }];
  ...
  describe('zebra striping', () => {
    it('dims alternate (odd-indexed) rows when zebra is true', () => {
      const columns: Column[] = [{ key: 'name', label: 'Name', width: 10 }];
      const rows = [{ name: 'Row0' }, { name: 'Row1' }, { name: 'Row2' }, { name: 'Row3' }];
      const result = renderTable({
        columns,
        rows,
        zebra: true,
      ...
      const columns: Column[] = [
        {
          ...
          label: 'Result',
          width: 15,
          format: (_v: any, row: Record<string, any>) => `${row.first} ${row.last}`,
        },
      ];
      ...
      const columns: Column[] = [
        {
          ...
          label: 'Status',
          width: 10,
          format: (v: string) => (v === 'ok' ? chalk.green('PASS') : chalk.red('FAIL')),
        },
      ];
  ...
  describe('alignment', () => {
    it('right-aligns values when align is right', () => {
      const columns: Column[] = [{ key: 'value', label: 'Value', width: 10, align: 'right' }];
      const rows = [{ value: '42' }];
      const result = renderTable({ columns, rows, maxWidth: 80 });
      const lines = result.split('\n');
    ...
    it('center-aligns values when align is center', () => {
      const columns: Column[] = [{ key: 'value', label: 'Value', width: 10, align: 'center' }];
      const rows = [{ value: 'Hi' }];
      const result = renderTable({ columns, rows, maxWidth: 80 });
      const lines = result.split('\n');
      const dataPlain = stripAnsi(lines[lines.length - 1]);
