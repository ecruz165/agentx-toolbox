...
describe('renderAvgOutputChart', () => {
  ...
  it('renders footer with marker explanation', () => {
    const bars: AvgOutputBar[] = [makeAvgBar('Platform', 4, { app: 1000 }, 800)];
    const result = renderAvgOutputChart({
      bars,
      segmentDefs,
      maxBarWidth: 40,
  ...
  it('uses custom marker char', () => {
    const bars: AvgOutputBar[] = [makeAvgBar('Platform', 4, { app: 800 }, 1200)];
    const result = renderAvgOutputChart({
      bars,
      segmentDefs,
      markerChar: '\u2666',
  ...
  it('displays formatted values when showValues is true', () => {
    const bars: AvgOutputBar[] = [makeAvgBar('Platform', 4, { app: 1300 }, 1100)];
    const result = renderAvgOutputChart({
      bars,
      segmentDefs,
      showValues: true,
  ...
  it('globalMax considers both totals and runningAvgs', () => {
    ...
    const bars: AvgOutputBar[] = [makeAvgBar('Team A', 3, { app: 500 }, 2000)];
    const result = renderAvgOutputChart({
      bars,
      segmentDefs,
      maxBarWidth: 50,
    ...
    const line = plain.split('\n')[0];
    const barChars = line.split('').filter((c) => c === '\u2588').length;
  ...
  it('renders all four segment types correctly', () => {
    const bars: AvgOutputBar[] = [
      makeAvgBar('Full', 5, { app: 400, test: 200, config: 100, storybook: 50 }, 600),
    ];
    const result = renderAvgOutputChart({
      bars,
      segmentDefs,
