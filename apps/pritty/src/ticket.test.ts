...
import { detectTicket, findRecentTicket, ticketLink, ticketPromptGuidance } from './ticket.js';
...
describe('ticketLink', () => {
  it('substitutes {ticket} into the template', () => {
    expect(ticketLink('PROJ-123', 'https://yourorg.atlassian.net/browse/{ticket}')).toBe(
  ...
  it('substitutes multiple {ticket} occurrences', () => {
    expect(ticketLink('PROJ-1', '{ticket}/edit/{ticket}')).toBe('PROJ-1/edit/PROJ-1');
  });
});
