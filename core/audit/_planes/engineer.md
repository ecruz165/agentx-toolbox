...
- 12b capability mismatch → run capability introduction
  workflow (`/workflows:manage start engineer:capability-introduction`)
...
Suggested next steps:
  /audit --fix
  /frameworks:heroui:build-components dashboard
...
```jsonc
{
  ...
  "suggestedActions": [ "/audit --fix", "/frameworks:heroui:build-components dashboard" ]
}
```
...
- It does not run visual regression. That's `/frameworks:heroui:build-components`.
