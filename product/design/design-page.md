...
2. **Per-page production design** (default `<page-type>` mode): A
   ...
   `/frameworks:heroui:build-components` reads.
...
- **Per-page mode** is the production-design entry point. Use after
  ...
  decomposition that `/frameworks:heroui:build-components` consumes.
...
5. /frameworks:heroui:build-components <page-slug>
```
...
📝 Next: per-page production design
   ...
   /frameworks:heroui:build-components <page-slug>  # generate React after design
```
