...
> `workflows/` namespace (`/workflows:manage`). Workflow playbooks
...
- `product/.pencil-marketing-calendar.json` — annual + monthly marketing calendar (produced by `/workflows:manage start market:marketing-calendar-annual` and updated by `market:marketing-calendar-monthly`)
- `product/.pencil-maintenance-calendar.json` — annual maintenance calendar with per-ecosystem cadence targets, compliance constraints, capacity, and cycle history (produced by `/workflows:manage start engineer:maintenance-calendar-annual` and updated by `engineer:polyglot-maintenance-cycle` runs)
...
framework namespace (e.g. `/frameworks:heroui:components:buttons`) only run
...
the same `/workflows:manage` state machine. This is the architectural
