...
Composes with `/core:frameworks:storybook:stories:gen` — this command
...
Strategy:
  ...
  2. Process sequentially via /core:frameworks:storybook:stories:gen
...
for COMPONENT in "${MISSING[@]}"; do
  ...
  # Delegate to gen
  /core:frameworks:storybook:stories:gen "$COMPONENT"
...
Failed (require manual investigation):
  components/core/molecules/breadcrumbs/Breadcrumbs.tsx
    Reason: Component shape unclear — exports default a
            ...
            Run /core:frameworks:storybook:stories:gen with explicit path
...
- `/core:frameworks:storybook:verify:health <category>` to verify generated
  stories actually render
- `/core:frameworks:storybook:catalog` to see updated coverage stats
- `/core:frameworks:storybook:stories:doc <component>` to add MDX docs to the
...
```bash
# In atomic-design.md
/core:frameworks:storybook:stories:gen-missing molecules
```
...
# Limit to atoms
/core:frameworks:storybook:stories:gen-missing atoms

# Limit to a feature section (manifest-defined)
/core:frameworks:storybook:stories:gen-missing "Discover Schools"

# Dry-run to see what would happen
/core:frameworks:storybook:stories:gen-missing --dry-run

# Cap batch size (work incrementally)
/core:frameworks:storybook:stories:gen-missing --limit 10

# Group by category in the report
/core:frameworks:storybook:stories:gen-missing --by-category
```
