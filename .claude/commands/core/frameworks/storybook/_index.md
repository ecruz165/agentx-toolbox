...
The complete catalog of `/core:frameworks:storybook:*` commands organized by
...
└── Yes → /core:frameworks:storybook:init

Authoring (deliberate creation):
├── Generate a story for a specific component       → /core:frameworks:storybook:stories:gen
├── Find components without stories, generate them  → /core:frameworks:storybook:stories:gen-missing
└── Generate MDX documentation for a component      → /core:frameworks:storybook:stories:doc

Verification (validate current state):
├── Check whether stories render without errors     → /core:frameworks:storybook:verify:health
├── Capture screenshot of a story                   → /core:frameworks:storybook:verify:screenshot
├── Investigate and fix a broken story              → /core:frameworks:storybook:verify:fix
├── Audit accessibility via axe-core                → /core:frameworks:storybook:verify:a11y
├── Run interaction (play function) tests           → /core:frameworks:storybook:verify:interactions
└── Verify storybook is ready to deploy             → /core:frameworks:storybook:verify:deploy

Migration (framework-version-specific transition work):
├── Verify a story migrated correctly (5 loops)     → /core:frameworks:storybook:migration:verify
└── Document or look up a fix pattern               → /core:frameworks:storybook:migration:fix-pattern

Reporting:
├── Full inventory + coverage + addon usage         → /core:frameworks:storybook:catalog
└── Visual regression (Chromatic) integration       → /core:frameworks:storybook:chromatic
...
### `/core:frameworks:storybook:stories:doc`
...
### `/core:frameworks:storybook:verify:health`
...
### `/core:frameworks:storybook:verify:fix`
...
### `/core:frameworks:storybook:verify:a11y`
...
### `/core:frameworks:storybook:verify:deploy`
...
### `/core:frameworks:storybook:migration:fix-pattern`
...
### `/core:frameworks:storybook:catalog`
...
### `/core:frameworks:storybook:chromatic`
...
### `/core:frameworks:storybook:init`
...
fields. Run `/core:frameworks:storybook:init` when:
