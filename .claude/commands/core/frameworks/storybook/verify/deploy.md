...
Before deploying:
  1. Investigate the blank story (run /core:frameworks:storybook:verify:fix
     core-atoms-button--with-icon)
...
/core:frameworks:storybook:verify:deploy --strict

# Skip rebuild (validate existing build)
/core:frameworks:storybook:verify:deploy --no-rebuild

# Specific deployment target (informational only currently)
/core:frameworks:storybook:verify:deploy --target staging
```
