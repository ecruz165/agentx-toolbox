...
> whenever any `/core:frameworks:storybook:migration:*` command runs.
>
...
- **`/core:frameworks:storybook:migration:verify`** — orchestrated
...
- **`/core:frameworks:storybook:migration:fix-pattern`** — document
...
test -d "${SCREENSHOT_DIR}before/" || {
  ...
  echo "  /core:frameworks:storybook:verify:screenshot all --label before"
  echo ""
  echo "Then perform your migration work, then run:"
  echo "  /core:frameworks:storybook:verify:screenshot all --label after"
  echo "  /core:frameworks:storybook:migration:verify"
  exit 1
}
```
...
```bash
# 1. Capture before
/core:frameworks:storybook:verify:screenshot all --label before
...
# 3. Capture after
/core:frameworks:storybook:verify:screenshot all --label after

# 4. Verify
/core:frameworks:storybook:migration:verify all

# 5. For failures: investigate
/core:frameworks:storybook:migration:verify Button --debug
