...
2. Chromatic enabled in `.pencil-storybook.json`:
   ...
   if [ "$ENABLED" != "true" ]; then
     ...
     echo "Run /core:frameworks:storybook:init --update to detect"
...
4. CHROMATIC_PROJECT_TOKEN environment variable set:
   ...
   ```
5. Chromatic CLI available (`/core:tools:chromatic`):
   ```bash
   ...
   if [ "$CHROMATIC_CLI" != "true" ]; then
     echo "Chromatic CLI not installed. Run /core:tools:setup chromatic"
     exit 1
   fi
   ```
...
/core:frameworks:storybook:chromatic --baseline-only

# Unreviewed changes only
/core:frameworks:storybook:chromatic --unreviewed-only

# Specific branch
/core:frameworks:storybook:chromatic --branch feature/new-cards

# CI integration
/core:frameworks:storybook:chromatic --json > chromatic-health.json
```
