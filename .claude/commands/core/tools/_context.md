...
│
├── playwright.md                /core:tools:playwright (data plane + registry def)
├── pixelmatch.md                /core:tools:pixelmatch
├── imagemagick.md               /core:tools:imagemagick
├── chrome-devtools.md           /core:tools:chrome-devtools
├── chromatic.md                 /core:tools:chromatic
├── open-pencil.md               /core:tools:open-pencil
├── pencil-mcp.md                /core:tools:pencil-mcp (the MCP, distinct from CLI)
├── figma.md                     /core:tools:figma
├── biome.md                     /core:tools:biome
├── eslint.md                    /core:tools:eslint
├── maven.md                     /core:tools:maven
├── gradle.md                    /core:tools:gradle
├── npm.md                       /core:tools:npm (covers npm/pnpm/yarn detection)
├── terraform.md                 /core:tools:terraform
├── context7.md                  /core:tools:context7
│
...
INVOKE them. Treating them as `/core:tools:claude-code "<prompt>"`
...
When invoked as `/core:tools:<tool>`, the file is the slash command.
...
fallback behavior. Read by `/core:tools:setup` and `/core:tools:manifest`
...
# Quick check via /core:tools:manifest
...
if [ "$AVAILABLE" != "true" ]; then
  ...
  echo "Run /core:tools:setup ${TOOL_NAME} to install."
  exit 1
fi
...
`/core:tools:setup` having ensured availability. Pre-flight checks
...
3. For missing tools: prompts to run `/core:tools:setup <tool>` or
...
When a tool's verification is stale, `/core:tools:setup` re-checks.
...
Categories help `/core:tools:setup` group tools during reporting
...
directly invoked by users via `/core:tools:<tool>` for custom
prompts. They're user-facing capabilities, persona-orthogonal
(every persona might invoke `/core:tools:playwright` for a one-off
...
- **Bypassing the manifest in commands** — commands that
  ...
  provides.
- **Auto-installing tools without user consent** — `/core:tools:setup`
