...
1. Verify chromatic active in tools manifest:
   ...
   if [ "$ACTIVE" != "true" ]; then
     echo "Chromatic CLI not active. Run /core:tools:setup chromatic"
     exit 1
   fi
   ```
...
/core:tools:chromatic "push the current Storybook build"

# Auto-accept (initial setup or sweeping intentional change)
/core:tools:chromatic "push and auto-accept all changes"

# Push for a feature branch
/core:tools:chromatic "push as branch feature/new-cards"

# Query latest build
/core:tools:chromatic "show the latest Chromatic build status"
```

Most users will invoke the higher-level
`/core:frameworks:storybook:chromatic` command (which wraps this
tool with project-aware logic). Direct `/core:tools:chromatic`
