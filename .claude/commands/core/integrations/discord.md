...
1. Verify `discord` integration is active:
   ...
   if [ "$ACTIVE" != "true" ]; then
     echo "Discord integration not active. Run /core:integrations:setup discord"
     exit 1
   fi
   ```
...
2. Verify bot token available:
   ...
   if [ -z "$BOT_TOKEN" ]; then
     echo "Discord bot token missing. Run /core:integrations:setup discord"
     exit 1
   fi
   ```
...
- **`/core:workflows:manage start engineer:adr-cycle`** — ADR
...
/core:integrations:discord "list my Discord servers"

# Read recent messages
/core:integrations:discord "show recent messages in #development on SkoolScout Engineering"

# Post update
/core:integrations:discord "post to #development on SkoolScout Engineering: PR #1247 ready"

# Post embed
/core:integrations:discord "post a rich embed to #incidents about the staging outage"

# Create thread
/core:integrations:discord "create a thread under that message titled 'PR review discussion'"

# Webhook post (CI integration)
/core:integrations:discord --webhook deploys "post: deploy to staging succeeded"
```
