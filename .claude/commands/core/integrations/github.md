...
1. Read `product/.pencil-integrations.json`. Verify `github` is
   ...
   if [ "$ACTIVE" != "true" ]; then
     echo "GitHub integration not active. Run /core:integrations:setup github"
     exit 1
   fi
   ```
...
|-------|--------------|----------------|
| 401 Unauthorized | Token expired/revoked | Re-run `/core:integrations:setup github` |
...
/core:integrations:setup --refresh-required-by when commands exist.)
