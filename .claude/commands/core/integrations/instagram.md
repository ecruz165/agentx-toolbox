...
1. Verify integration is active:
   ...
   if [ "$ACTIVE" != "true" ]; then
     echo "Instagram not active. Run /core:integrations:setup instagram"
     exit 1
   fi
   ```
   ...
   case "$ROUTE" in
     direct)
       ...
       [ -n "$IG_ACCESS_TOKEN" ] || {
         ...
         echo "Run /core:integrations:setup instagram"
         exit 1
       }
       ;;
     hootsuite)
...
Then internally invokes `/core:integrations:hootsuite` with the
...
Insights will be available in ~15 minutes via:
  /core:integrations:instagram "show insights for the last post"
```
...
|-------|--------------|----------------|
| 190 OAuthException | Token expired or revoked | Re-auth: /core:integrations:setup instagram |
...
```bash
# Account info
/core:integrations:instagram "show my Instagram account info"

# Post insights
/core:integrations:instagram "insights for my last post"

# Recent posts
/core:integrations:instagram "show my recent 10 posts with engagement"

# Comments
/core:integrations:instagram "show comments on my latest post"

# Publish (delegates to Hootsuite when configured)
/core:integrations:instagram "publish this post: [caption] with [image-path]"

# Force direct publish (skip delegation)
/core:integrations:instagram --use-direct-write \
  "publish this post: [caption] with [image-url]"

# Schedule via delegation
/core:integrations:instagram "schedule this post for tomorrow 9am: [caption]"
```
