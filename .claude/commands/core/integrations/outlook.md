...
1. Read `product/.pencil-integrations.json`. Verify `outlook`
   ...
   if [ "$ACTIVE" != "true" ]; then
     echo "Outlook integration not active. Run /core:integrations:setup outlook"
     exit 1
   fi
   
   if [ "$AUTH_PROVIDER" != "microsoft-graph" ]; then
     echo "Outlook integration not using microsoft-graph auth provider"
     echo "Manifest may be misconfigured. Re-run /core:integrations:setup outlook"
     exit 1
   fi
   ```
...
2. Verify auth provider is healthy:
   ...
   if [ "$PROVIDER_ACTIVE" != "true" ]; then
     ...
     echo "Run /core:integrations:setup microsoft-graph"
     exit 1
   fi
   ```
...
To read a specific message:
  /core:integrations:outlook "show me the FERPA compliance email from Sarah"
```
...
|-------|--------------|----------------|
| 401 Unauthorized | Access token expired and refresh failed | Re-authenticate: /core:integrations:setup microsoft-graph |
...
- **`/core:workflows:manage start engineer:adr-cycle`** — when
  ADR cycle wants to email the proposal to stakeholders
  (when configured)
- **`/core:workflows:manage start product:ux:research`** (future)
...
- **Sync with non-Microsoft email.** Gmail, Apple Mail, etc.
  use different integrations (`/core:integrations:gmail` if
...
```bash
# Read inbox
/core:integrations:outlook "show me unread emails from the last 24 hours"

# Search
/core:integrations:outlook "find emails from sarah@example.com about FERPA"

# Send
/core:integrations:outlook "email sarah@example.com about the meeting being moved to 3pm"

# Calendar
/core:integrations:outlook "what's on my calendar tomorrow"

# Schedule meeting
/core:integrations:outlook "schedule a 30-minute meeting with sarah@example.com next Tuesday at 2pm titled 'Project review'"

# Find availability
/core:integrations:outlook "find a 30-minute slot when sarah@ and tech-lead@ are both free next week"
```
