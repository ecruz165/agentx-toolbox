...
1. Verify integration is active:
   ...
   if [ "$ACTIVE" != "true" ]; then
     echo "Splunk not active. Run /core:integrations:setup splunk"
     exit 1
   fi
   ```
   ...
   case "$PREFERENCE" in
     rest)
       ...
       [ -n "$SPLUNK_TOKEN" ] || {
         ...
         echo "Run /core:integrations:setup splunk --instance $INSTANCE"
         exit 1
       }
       ;;
     cli)
...
Suggested investigation:
  /core:integrations:splunk --instance security \
...
|-------|--------------|----------------|
| 401 | Token expired or invalid | Re-run /core:integrations:setup splunk --instance <name> |
...
/core:integrations:splunk --instance prod \
...
/core:integrations:splunk "search recent errors for the auth service"

# Cross-instance correlation (sequential queries)
/core:integrations:splunk \
  "show me errors AND security alerts for the auth service"
