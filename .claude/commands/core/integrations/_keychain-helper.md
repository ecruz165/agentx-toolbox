...
if [ -z "$JIRA_API_TOKEN" ]; then
  echo "Credential not found in keychain. Run /core:integrations:setup jira"
  exit 1
fi
...
if [ -z "$JIRA_EMAIL" ] || [ -z "$JIRA_API_TOKEN" ]; then
  echo "Jira credentials not found. Run /core:integrations:setup jira"
  exit 1
fi
```
...
| Credential not found | Setup not run, or credential deleted | Suggest `/core:integrations:setup <name>` |
