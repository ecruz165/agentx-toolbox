...
3. **Compare against the manifest's recorded hashes** from the last
   `/core:frameworks:heroui:build-components` run.
...
`/core:frameworks:heroui:build-components` with the visual gates. Audit prints the
...
⚠️  design/pages/dashboard.pen modified 2026-05-02
    ...
    Suggested: /core:frameworks:heroui:build-components dashboard
```
...
component code (manual, through `/core:frameworks:heroui:build-components`) or to
