...
`/core:workflows:manage start <domain>:<workflow>` to begin one with
...
`product/.pencil-workflow-state.json`. Use `/core:workflows:manage status`
to see where you are; `/core:workflows:manage resume` to pick up where
