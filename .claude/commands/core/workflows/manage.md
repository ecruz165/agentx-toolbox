...
etc.). This command — `/core:workflows:manage` — is the domain-agnostic
...
`<domain>/workflows/<workflow-slug>.md`. Run `/core:workflows:manage list`
to see all available workflows discovered across domains.

For backward compatibility, bare workflow names (e.g.
`/core:workflows:manage start greenfield`) still resolve when there's no
...
Run /core:workflows:manage start <domain>:<workflow-slug> to begin one.
```
...
When complete, run: /core:workflows:manage complete brand-foundation
```
...
Run /core:workflows:manage next for the next step.
```
...
When complete, run: /core:workflows:manage complete components:buttons
```
...
Decision: Do you want to run competitive research before foundations?
  ...
  Yes — recommended for brand differentiation. Adds ~1 hour.
        ...
        Then: /core:workflows:manage complete research

  No  — skip to Phase 3 (foundations driven by brief alone)
        Run: /core:workflows:manage complete research --skip
```
...
⏸  Workflow paused: greenfield-2026-05-02-100000
   ...
   Resume with: /core:workflows:manage resume
```
...
Run /core:workflows:manage inspect <workflow-id> for details.
```
...
declares a decision, `/core:workflows:manage next` prompts the user:
...
Run: /core:frameworks:heroui:components:forms
Then: /core:workflows:manage complete components:forms

Or: /core:workflows:manage status to see overall progress
```
...
Run /core:workflows:manage list for full descriptions.
```
...
⚠️  An active workflow already exists:
    ...
    Choose:
      [c] Complete the active workflow first (run /core:workflows:manage status)
