...
Strategy-domain workflows (`product/strategy/workflows/`):
...
Design-domain workflows (`product/design/workflows/`):

- `design:figma-roundtrip` — designer-in-Figma iteration
  loop
- `design:migrate-from-figma` — bring Figma design system
  into Pencil
- `design:migrate-to-pencil` — bring existing product UI
  into Pencil
...
primary-domain placement rule, they live in their respective
sub-namespace.

When invoked: `/core:workflows:manage start <domain>:<workflow-slug>`.
...
```
/core:workflows:manage start product:brownfield-add-feature
```
...
```
/core:workflows:manage start product:brand-refresh
```
...
- **Product workflows invoking marketing or engineer
  ...
  `/core:workflows:manage start market:launch-campaign`; resume."
