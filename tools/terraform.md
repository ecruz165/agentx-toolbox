...
1. Verify terraform active in tools manifest:
   ...
   if [ "$ACTIVE" != "true" ]; then
     echo "Terraform not active. Run /tools:setup terraform"
     exit 1
   fi
   ```
...
```bash
# Initialize
/tools:terraform "initialize the working directory"

# Plan changes
/tools:terraform "plan changes"

# Apply (with confirmation)
/tools:terraform "apply the plan"

# Plan a specific resource
/tools:terraform "plan changes to the api ECS service" \
  -target=aws_ecs_service.api

# Show state
/tools:terraform "show me what's in state"

# Switch workspace
/tools:terraform "switch to staging workspace"

# Format check
/tools:terraform "check formatting of all tf files"
```
