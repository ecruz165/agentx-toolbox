...
1. Verify biome active in tools manifest:
   ...
   if [ "$ACTIVE" != "true" ]; then
     echo "Biome not active. Run /tools:setup biome"
     echo ""
     echo "If your project uses ESLint instead, run /tools:setup eslint"
     exit 1
   fi
   ```
...
- **Replace ESLint when ESLint is the project's choice.** If
  the project has .eslintrc, use `/tools:eslint`.

## Examples

```bash
# Lint check (no changes)
/tools:biome "check lint issues in app-ui/components"

# Apply safe auto-fixes
/tools:biome "fix all auto-fixable lint issues" --fix

# Apply including unsafe fixes
/tools:biome "fix all lint including potentially behavior-changing" --apply-unsafe

# Format-only check
/tools:biome "show me files that need formatting" 

# Combined check (most common)
/tools:biome "run full check on app-ui" --fix
```
