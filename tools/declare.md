...
the dual-purpose pattern: command body for `/tools:<tool>`
...
```
How rich should /tools:<tool-name> direct invocation be?
...
1. Read `product/.pencil-tools.json`. If <tool-name> isn't
   available, surface and offer to run `/tools:setup
   <tool-name>`.
...
> `/tools:setup` and `/tools:manifest`.
...
<empty initially; populated by /tools:setup --refresh-required-by>
...
Next steps:
  ...
  - If applicable, document cross-references to skillz commands
    that wrap this tool
  - Run /tools:setup <tool-name> to install/verify and update
    the runtime manifest
  - When commands that use this tool exist, run
    /tools:setup --refresh-required-by to populate the
    required-by relationships
```

## What this command does NOT do

- **Install the tool.** That's `/tools:setup`'s job.
...
- **Auto-detect existing tool usage in the suite.** The
  ...
  `/tools:setup --refresh-required-by` after commands using
  the tool exist.
...
/tools:declare prettier

# Declare a tool with both CLI and MCP
/tools:declare puppeteer --cli-and-mcp

# Declare an ecosystem-specific tool
/tools:declare cargo --ecosystem-specific rust
```
