...
1. Verify maven active in tools manifest:
   ...
   if [ "$ACTIVE" != "true" ]; then
     echo "Maven not active. Run /tools:setup maven"
     exit 1
   fi
   ```
...
```bash
# Standard build
/tools:maven "clean package"

# Skip tests (upgrade verification)
/tools:maven "clean package -DskipTests"

# Show outdated dependencies
/tools:maven "show me available dependency updates"

# Run OpenRewrite Spring Boot 3 upgrade
/tools:maven "run OpenRewrite Spring Boot 3 upgrade recipe"

# Module-scoped build
/tools:maven "build only the api module" --module api

# Dependency tree with conflicts
/tools:maven "show me dependency tree with conflicts"
```
