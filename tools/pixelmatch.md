...
1. Verify pixelmatch active in tools manifest:
   ...
   if [ "$ACTIVE" != "true" ]; then
     echo "pixelmatch not active. Run /tools:setup pixelmatch"
     exit 1
   fi
   ```
...
pixelmatch requires same dimensions. Either:
  ...
  - Resize before comparing (use ImageMagick: /tools:imagemagick)
```
...
- **Capture screenshots.** That's Playwright (`/tools:playwright`).
  pixelmatch only compares existing PNG files.
- **Resize images.** Use ImageMagick (`/tools:imagemagick`)
...
```bash
# Direct comparison
/tools:pixelmatch before.png after.png

# With threshold and diff output
/tools:pixelmatch before.png after.png /tmp/diff.png --threshold 0.05

# As part of migration verification (programmatic)
# (Invoked by /frameworks:storybook:migration:verify)
```
