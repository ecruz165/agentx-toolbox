...
1. Verify eslint active in tools manifest:
   ...
   if [ "$ACTIVE" != "true" ]; then
     echo "ESLint not active. Run /tools:setup eslint"
     echo ""
     echo "If your project uses Biome instead, run /tools:setup biome"
     exit 1
   fi
   ```
...
Run /tools:biome 'check format' OR /tools:prettier (when added)
...
/tools:eslint "check lint issues in app-ui/components"

# Apply auto-fixes
/tools:eslint "fix auto-fixable issues" --fix

# Strict mode (fail on warnings)
/tools:eslint "lint in strict mode" --max-warnings 0

# Quiet (errors only)
/tools:eslint "show only errors" --quiet

# Specific rule diagnosis
/tools:eslint "find all places using `any` type"
