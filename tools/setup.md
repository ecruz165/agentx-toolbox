...
```bash
/tools:setup --scope browser-automation
```
...
```bash
/tools:setup --refresh-required-by
```
...
Next steps:
  - Run /tools:manifest to see current state
  - Run /tools:setup --check periodically to verify availability
