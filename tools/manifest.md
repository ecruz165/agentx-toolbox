...
> No tools manifest exists. Run `/tools:setup` to create one.
...
playwright (required)            ✓ CLI v1.40.1
                                 ...
                                 Run /tools:setup --check to re-verify.
```
...
```bash
/tools:manifest --query "tools.playwright.interfaces.cli.available"
# → true

/tools:manifest --query "tools.playwright.interfaces.mcp.serverName"
# → "playwright"

/tools:manifest --query "tools | keys"
# → ["biome", "chromatic", "chrome-devtools", ...]

/tools:manifest --query "tools | to_entries | map(select(.value.optional == false and (.value.interfaces.cli.available == false))) | length"
...
```bash
/tools:manifest --mark-optional chromatic
```
...
```bash
/tools:manifest --mark-required imagemagick
```
...
- **Stale** (verification > 30 days ago): visible warning (⚠)
  with suggested action `/tools:setup --check`
...
- **Install or remove tools.** That's `/tools:setup`'s job.
- **Detect new tools.** Detection happens in `/tools:setup`
  during verification.
...
- **Re-verify availability.** It reads the cached state.
  Re-verification requires `/tools:setup --check`.
...
/tools:manifest --query "tools.playwright.interfaces.mcp.available"

# Find which commands depend on imagemagick
/tools:manifest --query "tools.imagemagick.requiredBy"

# Mark a tool optional for this project
/tools:manifest --mark-optional chromatic

# List required tools that are currently missing
/tools:manifest --query "tools | to_entries | map(select(.value.optional == false and (.value.interfaces.cli.available == false))) | map(.key)"
```
