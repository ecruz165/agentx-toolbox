...
`/integrations:<name>` direct invocation + registry definition
...
> `/integrations:setup` and `/integrations:manifest`.
...
<empty initially; populated by /integrations:setup --refresh-required-by>
...
Next steps:
  ...
  - Run /integrations:setup linear to configure auth and
    test connectivity
  - When commands that use this integration exist, run
    /integrations:setup --refresh-required-by to populate
    the required-by relationships
```

## What this command does NOT do

- **Configure auth.** That's `/integrations:setup`'s job.
...
/integrations:declare linear

# Specify interfaces upfront
/integrations:declare linear --cli --mcp --rest

# Compliance-sensitive declaration
/integrations:declare custom-internal-jira \
  --provider atlassian \
  --all-interfaces
```
