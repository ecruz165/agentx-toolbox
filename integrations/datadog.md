...
1. Verify integration is active:
   ...
   if [ "$ACTIVE" != "true" ]; then
     echo "Datadog not active. Run /integrations:setup datadog"
     exit 1
   fi
   ```
   ...
   case "$PREFERENCE" in
     ...
     rest)
       ...
       [ -n "$DD_API_KEY" ] && [ -n "$DD_APP_KEY" ] || {
         ...
         echo "Run /integrations:setup datadog"
         exit 1
       }
       ;;
   esac
...
ALERT (1):
  [Monitor #12345] API error rate > 5%
    ...
    Action: /integrations:datadog "show me logs from the last 30 minutes for skoolscout-api errors"
...
- **`/integrations:teams` or `/integrations:discord`** —
...
/integrations:datadog "show me current alerts for skoolscout-api"

# Recent errors
/integrations:datadog "find error logs from skoolscout-api in the last 30 minutes"

# Performance investigation
/integrations:datadog "what's the p95 latency on the auth endpoint over the last 6 hours"

# Active incidents
/integrations:datadog "list active incidents"

# Mute monitor for maintenance
/integrations:datadog "mute monitor 12345 for 2 hours during deployment"

# Deployment marker (CLI path)
/integrations:datadog "mark deployment of v1.4.2 to staging" --use-cli
```
