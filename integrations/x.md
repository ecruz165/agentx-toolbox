...
[ "$ACTIVE" = "true" ] || {
  echo "X (Twitter) not active. Run /integrations:setup x"
  exit 1
}
...
if [ "$OPERATION_TYPE" = "write" ]; then
  ...
  if [ "$HOOTSUITE_ACTIVE" = "true" ] && [ "$X_MANAGED" = "true" ] && \
    ...
    if [ "$API_TIER" = "free" ]; then
      ...
      echo "  Consider Hootsuite delegation: /integrations:setup hootsuite"
    fi
  fi
else
  ROUTE="direct"
...
Search limited on free tier. Upgrade tier for better access:
  /integrations:setup x  # configure API tier
```
...
```bash
# Account info
/integrations:x "show my X account stats"

# Recent posts
/integrations:x "show my recent posts with engagement"

# Search
/integrations:x "search for FERPA compliance discussions in the last 24 hours"

# Mentions
/integrations:x "show recent mentions of @skoolscout"

# Post (delegates to Hootsuite when configured)
/integrations:x "post: Excited to announce..."

# Post thread
/integrations:x "post a 4-tweet thread about FERPA compliance: [content]"

# Reply
/integrations:x "reply to that post about FERPA prep time with helpful context"

# Force direct posting
/integrations:x --use-direct-write "post: ..."
```
