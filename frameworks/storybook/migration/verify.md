...
[ "$PLAYWRIGHT" = "true" ] || { echo "Playwright required. Run /tools:setup playwright"; exit 1; }
[ "$PIXELMATCH" = "true" ] || { echo "pixelmatch required. Run /tools:setup pixelmatch"; exit 1; }
...
core-atoms-button--ghost
  ...
  Suggested next step:
    /frameworks:storybook:migration:fix-pattern Button --gotcha-id heroui-v3-button-ghost-radius
...
Next steps:
  ...
  - For known patterns: /frameworks:storybook:migration:fix-pattern <component>
  - For new patterns: --add-gotcha to document
  - Re-run after fixes: /frameworks:storybook:migration:verify
```
...
/frameworks:storybook:migration:verify Button --from-loop 2
```
...
```bash
/frameworks:storybook:migration:verify Button--ghost --debug
```
...
# Specific component
/frameworks:storybook:migration:verify Button

# Specific story
/frameworks:storybook:migration:verify core-atoms-button--ghost

# Strict mode (no WARN tolerance)
/frameworks:storybook:migration:verify --strict

# Skip color (intentional brand update)
/frameworks:storybook:migration:verify --skip-color

# Resume from Loop 3 (you fixed earlier loops)
/frameworks:storybook:migration:verify Button --from-loop 3
