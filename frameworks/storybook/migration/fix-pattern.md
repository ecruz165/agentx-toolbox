...
```bash
/frameworks:storybook:migration:fix-pattern Button
```
...
```bash
/frameworks:storybook:migration:fix-pattern --symptom "border-radius"
```
...
```bash
/frameworks:storybook:migration:fix-pattern --gotcha-id heroui-v3-button-ghost-radius
```
...
```bash
/frameworks:storybook:migration:fix-pattern --list
```
...
```bash
/frameworks:storybook:migration:fix-pattern --remove heroui-v3-button-ghost-radius
```
...
core-atoms-button--ghost
  ...
  Matching gotcha:
    [from /frameworks:storybook:migration:fix-pattern lookup]
    heroui-v3-button-ghost-radius
    ...
```
...
- **Replace official upgrade guides.** Gotchas supplement
  ...
  (`/tools:context7`) is the way to look up official docs.
...
/frameworks:storybook:migration:fix-pattern --remove heroui-v2-button-padding
```
