...
> `/frameworks:storybook:*` command runs. Sub-namespace `_context.md`
...
harness. The `/frameworks:storybook:*` commands cover its full lifecycle:
...
- **Maintenance routines** — `atomic-design.md` invokes
  `/frameworks:storybook:stories:gen-missing` for AD-7 violations;
...
This means users don't have to run `/frameworks:storybook:init` explicitly
...
`/frameworks:storybook:init` directly. This walks through every manifest
...
integration is available via `/frameworks:storybook:chromatic` for
...
`/frameworks:storybook:verify:*` and other diagnostic commands.
