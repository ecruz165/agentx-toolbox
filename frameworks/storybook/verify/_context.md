...
> whenever any `/frameworks:storybook:verify:*` command runs.
>
...
- **`/frameworks:storybook:verify:health`** — verify stories
  render without errors (PASS / LOADING / BROKEN / BLANK)
- **`/frameworks:storybook:verify:screenshot`** — capture
  story screenshots (standalone or as building block)
- **`/frameworks:storybook:verify:fix`** — investigate and fix
  broken stories
- **`/frameworks:storybook:verify:a11y`** — accessibility audit
  via axe-core
- **`/frameworks:storybook:verify:interactions`** — run
  interaction (play function) tests
- **`/frameworks:storybook:verify:deploy`** — verify build is
  ready to deploy
...
if [ "$ACTIVE" != "true" ]; then
  ...
  echo "Run /frameworks:init to detect and activate, or"
  echo "/frameworks:manifest --reactivate storybook if previously deactivated."
  exit 1
fi

# Check storybook deep config exists
test -f product/.pencil-storybook.json || {
  echo "Storybook deep config missing. Running /frameworks:storybook:init"
...
- **Playwright** (`/tools:playwright`) — browser automation for
  screenshots, interactions, page queries
- **Chrome DevTools MCP** (`/tools:chrome-devtools`, when
...
if [ "$PLAYWRIGHT_CLI" != "true" ]; then
  echo "Playwright is required for verification. Run /tools:setup playwright"
  exit 1
fi
```
