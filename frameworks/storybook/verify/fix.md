...
| MISSING: story file absent | Suggest `/frameworks:storybook:stories:gen ComponentName` |
...
core-atoms-rating--readonly
  ...
  Suggested: Run /frameworks:storybook:init --update-addons
...
Next steps:
  - Re-run /frameworks:storybook:verify:health to confirm
...
- **Install missing addons.** Surfaces the gap; install is
  user's call (or `/frameworks:storybook:init --update-addons`).
- **Generate missing stories.** Routes to
  `/frameworks:storybook:stories:gen` when classification is
  MISSING.
- **Handle framework-version-migration drift.** That's
  `/frameworks:storybook:migration:fix-pattern`. This command
...
/frameworks:storybook:verify:fix core-atoms-button--with-icon

# Fix all broken stories in a component
/frameworks:storybook:verify:fix Button

# Fix everything broken (use after verify:health)
/frameworks:storybook:verify:fix all

# Investigation only (no auto-fix)
/frameworks:storybook:verify:fix core-atoms-button--with-icon --no-apply

# Deeper investigation (used in drift cycles)
/frameworks:storybook:verify:fix all --strict-investigation
```
