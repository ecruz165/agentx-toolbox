...
1. Read `product/.pencil-frameworks.json`. If missing:
   - In query/show mode: report "no framework manifest exists;
     run `/frameworks:init` to create one."
...
storybook (active)
    Verified:      2026-04-15 (18 days ago) ⚠ stale
                   Run /frameworks:init --check to re-verify.
```
...
```bash
/frameworks:manifest --query componentBindings.heroui.active
# → true

/frameworks:manifest --query componentBindings.heroui.version
# → "3.0.0"

/frameworks:manifest --query "documentationBindings | keys"
# → ["storybook"]

/frameworks:manifest --query "componentBindings | to_entries | map(select(.value.active)) | length"
# → 1
```
...
- Reports cross-binding implications: "Deactivating storybook;
  /frameworks:storybook:* commands will refuse to run until
  reactivation."
...
Deactivating means:
  - All /frameworks:storybook:* commands will refuse to run
  ...
  - Detection signals are preserved; reactivate via
    /frameworks:manifest --reactivate storybook
...
- Suggests running `/frameworks:init --check --binding <name>`
...
```bash
# Pre-flight in /frameworks:storybook:stories:gen
...
- **Detect new bindings.** That's `/frameworks:init`'s job.
- **Verify values against project state.** That's
  `/frameworks:init --check`'s job.
...
/frameworks:manifest --query documentationBindings.storybook.active

# Periodic review (typical: every 6 months)
/frameworks:manifest --review
